...
train_features, test_features, train_labels, test_labels = train_test_split(
    ...
    test_size=0.2,
    random_state=41,
	stratify=regards_data[target_column])
...
if(classifier_name == "PA" or classifier_name == "LARS"):
	train_prediction = clf.predict(train_features[filtered_features].fillna(0))
	results_file.write('\nROC-AUC on training set: {}'.format(roc_auc_score(train_labels, train_prediction)))

	test_prediction = clf.predict(test_features[filtered_features].fillna(0))
	results_file.write('\nROC-AUC on test set: {}'.format(roc_auc_score(test_labels, test_prediction)))
